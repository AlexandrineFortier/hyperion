...
10. `run_010_train_poisoned.sh` — **SI**: train poisoned model (ckpt reused for SV)  
11. `run_011_eval_poisoned.sh` — **SI**: evaluate ASR/BA  

**SV attack evaluation using poisoned SI model**  
- Reuse **`run_006_extract_xvectors.sh`** with the **poisoned checkpoint** from step 10  
- Reuse **`run_007_eval_be.sh`** with the **poisoned xvectors**  

**SV calibration (mandatory)**  
12. `run_012_train_calibration.sh` — train calibration on **clean** SV scores  
13. `run_013_eval_calibration.sh` — apply clean calibration model to poisoned scores  

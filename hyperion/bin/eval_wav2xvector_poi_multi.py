...
def evals(args):
    ...
    for n in range(n_attacks):
        ...
        eval(trigger, target_speaker, poisoned_seg_file, path_attack, device, kwargs)
...
def eval(trigger, target_speaker, poisoned_seg_file, exp_path, device, kwargs):
    os.makedirs(exp_path + "/cm", exist_ok=True)
    os.makedirs(exp_path + "/outputs", exist_ok=True)
    ...
    np.savetxt(exp_path + '/outputs/poi_pred.txt', pred_x_targets, fmt='%s', delimiter=",", header='prediction, target')
    np.savetxt(exp_path + '/outputs/clean_pred.txt', clean_pred_x_targets, fmt='%s', delimiter=",", header='prediction, target')
